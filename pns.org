...
*** DONE use new tree format
    ...
    (defun pns-create-template-table (&optional mode)
      "Create a name to template object table(list of cons), given template. Key is name, value is template. this table can be used as helm candidates. MODE is a regexp to match the mode(if nil the for all modes)"
      (or mode (setq mode ".*"))
      (let (rst)
...
*** 190128 add an action for an snippet: go to the source org file of a snippet
    When running pns-expand-template-by-name
    Curruently there is only one action: expand the snippet.
    Go to the source org file will be helpful. You can see doc there (DEMO VERSION!) or modify the snippet.
*** 190128 think of a way to adding snippet efficiently
    ...
    The good is that after the updateion, the new snippet will be avaliable on next run
