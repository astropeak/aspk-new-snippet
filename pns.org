...
With optional prefix argument ARG, jump forward ARG many source blocks."
  ...
  (org-next-block arg nil org-babel-src-block-regexp))
    ...
    ;; (setq pns-templates (pns-assoc 'header (nth 1 pns-tmp-templates)))



    (defun pns-create-templates (blocks c-level c-header c-content)
      (when block
        (let* ((block (car blocks))
               (level (pns-assoc 'level block))
               (name (pns-assoc 'name block))
               (content (pns-assoc 'content) block))
          (when block
            (if (> level c-level)
                (cond ((string-equal name "header")
                       (pns-create-templates (cdr blocks) level (append c-header content) c-content))
                      ((string-equal name "content")
                       (pns-create-templates (cdr blocks) level c-header (append c-content content))))
              ;; this is a leaf
              ;; 不是tree结构的，很难做递归。此时处理完这个block后，处理后续blocks时， 不知道当前的c-header所有的状态变量。
              (cond ((string-equal name "header")
                     (pns-create-templates (cdr blocks) level (append c-header content) c-content))
                    ((string-equal name "content")
                     (pns-create-templates (cdr blocks) level c-header (append c-content content))))
