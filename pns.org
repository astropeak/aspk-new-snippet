...
**** read all shortcuts from a snippet file
     ...
     (defun pns-create-shortcuts (file)
       ...
       (mapcar (lambda (src-block)
               ...
               (pns-get-all-src-blocks file pns-shortcut-entry-regexp)))
...
*** STARTED new idea: delete shortcut, create expand env dynamically and look backward, every headlines are selectable
    - State "STARTED"    from ""           [2017-01-06 Fri 15:08]
      Already have many thinkings, writting many notes.
    1. headlines selection:
    ...
    Some findings:
       ...
       - template: assoc list of below element. Represent an org mode file entry.
         ...
         - defined variables: list of list of variable name and their possiable values.
           All variables defined, either formlly by @(vars) annotation, or by embedded syntax `var value`.
           Order is respected.
         - hungry variables: list of variable names, which is a string. This is the variables that need getting value, either form env, or end user.
           either specified by @(get) annotation, or by embedded syntax `var value`.
           Order is respected.
         - precondition variables: assoc list of ((equal . (A)) (notequal . (A))), A: list of (var value).
      ...
      2. [optional]already filled variables: list of cons of (varname . value)
...
**** the new parser
     ...
     #+name: test-input 
     #+BEGIN_SRC org :tangle /tmp/pns.x.y/1.org :mkdirp yes
     ,* snippet: aaa
       ,#+name: header
       ,#+begin_src elisp
