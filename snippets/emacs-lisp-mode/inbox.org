...
** hwo to use it to capture a code snippet
  ...
  "* TODO %?\n....": template. The templete of this entry.
   ...
   So to capture a code snippet, we can just write a function to generate the template dynamically. The target we (DEMO VERSION!) can use (file+headline), because (DEMO VERSION!) we will always put the new snippets to all-mode/inbox.org


   Hatch code:

   a function to (DEMO VERSION!) create template.
   #+begin_src emacs-lisp
